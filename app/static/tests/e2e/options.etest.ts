...
test.describe("Options Tab tests", () => {
  ...
  test("cleared parameter input resets on model run", async ({ page }) => {
    ...
    await expect(await page.locator(":nth-match(#model-params input, 1)")).toHaveValue("4", {
      timeout
    });
  });
  ...
  test("can swap a parameter set and its run traces", async ({ page }) => {
    ...
    await expect(await page.innerText(":nth-match(.parameter-set .card-body span.badge, 3)")).toBe("sigma: 1.5");
  ...
  const expectAdvancedSetting = async (page: Page, index: number) => {
    ...
    await expect(await page.innerText(`:nth-match(#advanced-settings-panel label, ${index + 1})`)).toBe(
      advancedOptions[index].label
    );
