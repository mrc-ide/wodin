...
test.describe("Options Tab tests", () => {
  ...
  test("can see default parameters", async ({ page }) => {
    await expect(await page.innerText(".collapse-title")).toContain(
      "Model Parameters"
    );
    await expect(
      await page.getAttribute(":nth-match(.collapse-title i, 1)", "data-name")
    ).toBe("chevron-up");
    await expect(await page.locator("#model-params")).not.toBeHidden();

    await expect(
      await page.innerText(":nth-match(#model-params label, 1)")
    ).toBe("beta");
    await expect(
      await page.inputValue(":nth-match(#model-params input, 1)")
    ).toBe("4");
    await expect(
      await page.innerText(":nth-match(#model-params label, 2)")
    ).toBe("I0");
    await expect(
      await page.inputValue(":nth-match(#model-params input, 2)")
    ).toBe("1");
    await expect(
      await page.innerText(":nth-match(#model-params label, 3)")
    ).toBe("N");
    await expect(
      await page.inputValue(":nth-match(#model-params input, 3)")
    ).toBe("1,000,000");
    await expect(
      await page.innerText(":nth-match(#model-params label, 4)")
    ).toBe("sigma");
    await expect(
      await page.inputValue(":nth-match(#model-params input, 4)")
    ).toBe("2");

    await expect(
      await page.innerText(":nth-match(.collapse-title, 2)")
    ).toContain("Run Options");
    await expect(
      await page.getAttribute(":nth-match(.collapse-title i, 2)", "data-name")
    ).toBe("chevron-up");
    await expect(await page.locator("#run-options")).not.toBeHidden();

    await expect(await page.locator("#run-options label").count()).toBe(1);
    await expect(
      await page.innerText(":nth-match(#run-options label, 1)")
    ).toBe("End time");
    await expect(
      await page.inputValue(":nth-match(#run-options input, 1)")
    ).toBe("100");
  });
  ...
  test("can update parameter value", async ({ page }) => {
    ...
    const graphPathSelector =
      ":nth-match(.plotly svg .scatterlayer .scatter g.lines path, 1)";
    ...
    await expect(
      await page.locator(".run-tab .action-required-msg")
    ).toHaveText(
  ...
  test("can update end time", async ({ page }) => {
    ...
    await expect(await page.locator(xAxisTickSelector).last().innerHTML()).toBe(
      "100"
    );
  ...
  test("cleared parameter input resets on model run", async ({ page }) => {
    ...
    await expect(
      await page.locator(":nth-match(#model-params input, 1)")
    ).toHaveValue("4", {
      timeout,
    });
  });

  test("cannot enter chars in parameter input which are not numeric, comma or decimal point", async ({
    page,
  }) => {
  ...
  test("Entered parameter values are formatted when blur input", async ({
    page,
  }) => {
  ...
  test("Non-numbers are reverted to previous value when blur input", async ({
    page,
  }) => {
    await page.fill(":nth-match(#model-params input, 1)", ",.");
    await page.click(":nth-match(#model-params input, 2)");
  ...
  test("can change graph setting for log scale y axis", async ({ page }) => {
    await expect(
      await page.innerText(":nth-match(.collapse-title, 3)")
    ).toContain("Graph Settings");
  ...
  test("can create a parameter set, and see run traces for that set", async ({
    page,
  }) => {
    ...
    await expect(
      (await page.innerText(".parameter-set .card-header")).trim()
    ).toBe("Set 1");
    await expect(
      await page.innerText(
        ":nth-match(.parameter-set .card-body span.badge, 1)"
      )
    ).toBe("beta: 4");
    await expect(
      await page.innerText(
        ":nth-match(.parameter-set .card-body span.badge, 2)"
      )
    ).toBe("I0: 1");
    await expect(
      await page.innerText(
        ":nth-match(.parameter-set .card-body span.badge, 3)"
      )
    ).toBe("N: 1000000");
    await expect(
      await page.innerText(
        ":nth-match(.parameter-set .card-body span.badge, 4)"
      )
    ).toBe("sigma: 2");

    await page.click("#run-btn");
    await expect(
      await page.locator(".wodin-plot-data-summary-series")
    ).toHaveCount(6, { timeout });
    // current parameters
  ...
  test("can swap a parameter set and its run traces", async ({ page }) => {
    ...
    expect(page.getByText("sigma: 0")).toBeVisible();
  ...
  test("can get unique parameter set name after delete", async ({ page }) => {
    ...
    await expect(
      (
        await page.innerText(":nth-match(.parameter-set .card-header, 1)")
      ).trim()
    ).toBe("Set 1");
    await expect(
      (
        await page.innerText(":nth-match(.parameter-set .card-header, 2)")
      ).trim()
    ).toBe("Set 3");
  });
  ...
  test("error tooltip and doesn't change name if same name exists", async ({
    page,
  }) => {
    await createParameterSet(page);
    await editDisplayName(1, page);
    ...
    await saveDisplayName(1, page);
    await expect(
      (await page.innerText(":nth-match(.tooltip-inner, 2)")).trim()
    ).toBe("Name already exists");
    await expect(await page.isVisible(":nth-match(.param-name-input, 2)")).toBe(
      true
    );
  });

  test("error tooltip and doesn't change name if Set [number] format used", async ({
    page,
  }) => {
    await createParameterSet(page);
    await editDisplayName(1, page);
  ...
  const fillInAdvancedInputs = async (
    type: string,
    advancedSetting: any,
    index: number
  ) => {
  ...
  const expectAdvancedSetting = async (page: Page, index: number) => {
    ...
    await expect(
      await page.innerText(
        `:nth-match(#advanced-settings-panel label, ${index + 1})`
      )
    ).toBe(advancedOptions[index].label);
    const advancedSetting = await advancedSettingPanel.locator(
      `:nth-match(.row, ${index + 1})`
    );
    await fillInAdvancedInputs(
      advancedOptions[index].type,
      advancedSetting,
      index + 1
    );
    await expect(
