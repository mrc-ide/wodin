...
test.describe("Sessions tests", () => {
  ...
  test("can use Sessions page", async () => {
    ...
    await expect(await page.locator(":nth-match(.session-label, 4)")).toHaveText("current session label", { timeout });

    // ...then unchecking should filter out the earlier unlabelled duplicates again
    await page.uncheck("#show-duplicates-check");
    await expect(await page.locator(":nth-match(.session-label, 1)")).toHaveText(noLabel, { timeout });
    await expect(await page.locator(":nth-match(.session-label, 2)")).toHaveText("current session label", { timeout });
    ...
    await expect(await page.locator(":nth-match(.session-label, 3)")).toHaveText("current session label", { timeout });
