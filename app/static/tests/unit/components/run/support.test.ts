...
describe("construct actionable fit update messages from fit state changes", () => {
    ...
    it("shows fallback when no reason can be found", () => {
        expect(runRequiredExplanation(base)).toBe(
    ...
    it("shows sensible message when everything has changed", () => {
        ...
        expect(runRequiredExplanation(everything)).toBe(
            "Plot is out of date: model code has been recompiled, number of replicates has changed" +
    ...
    it("gives specific messages when little has changed", () => {
        ...
        expect(runRequiredExplanation({ ...base, modelChanged: true })).toBe(
            `${prefix}: model code has been recompiled. ${suffix}`
        );
        expect(runRequiredExplanation({ ...base, parameterValueChanged: true })).toBe(
            `${prefix}: parameters have been changed. ${suffix}`
        );
        expect(runRequiredExplanation({ ...base, endTimeChanged: true })).toBe(
            `${prefix}: end time has changed. ${suffix}`
        );
        expect(runRequiredExplanation({ ...base, numberOfReplicatesChanged: true })).toBe(
            `${prefix}: number of replicates has changed. ${suffix}`
        );
        expect(runRequiredExplanation({ ...base, advancedSettingsChanged: true })).toBe(
