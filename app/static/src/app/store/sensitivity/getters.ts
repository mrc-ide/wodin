...
export const getters: SensitivityGetters & GetterTree<SensitivityState, AppState> = {
    ...
    [SensitivityGetter.parameterSetSensitivityUpdateRequired]: (state: SensitivityState, _: SensitivityGetters,
        rootState: AppState) => {
        ...
        const missingResults = rootState.run.parameterSets
            .some((ps: ParameterSet) => !state.parameterSetResults[ps.name]?.batch);
