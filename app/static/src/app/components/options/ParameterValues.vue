  ...
  <div class="container">
    <div v-for="(paramName, index) in paramNames" class="row my-2" :key="paramKeys[index]">
      ...
      <div v-if="fitTabIsOpen" class="col-1">
        <input class="form-check-input align-bottom"
               type="checkbox"
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const paramsToVary = computed<string[]>(() => store.state.appType === AppType.Fit ? store.state.modelFit.paramsToVary : []);
        ...
        const checkBoxChange = (paramName: string, event: Event) => {
            ...
            const newParams = checked ? [...paramsToVary.value, paramName] : paramsToVary.value.filter((p) => p !== paramName);
