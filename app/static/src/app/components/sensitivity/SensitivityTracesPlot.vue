<template>
  <wodin-plot
    ...
    :redrawWatches="
      solutions ? [...solutions, allFitData, selectedVariables, parameterSetBatches, parameterSetDisplayNames] : []
    "
  >
    <slot></slot>
  </wodin-plot>
      ...
      if (solutions.value.length) {
        ...
        parameterSetNames.forEach((name) => {
          ...
          setSolutions.forEach((sln: OdinSolution, slnIdx: number) => {
            addSolutionOutputToResult(sln, plotlyOptions, (plotTrace) =>
              updatePlotTraceName(plotTrace, varyingParamName, parValues[slnIdx], currentParamSet!.displayName)
            );
          });
