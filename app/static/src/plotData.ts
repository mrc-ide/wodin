...
import { allFitDataToSkadiChart, discreteSeriesSetToSkadiChart, filterSeriesSet, filterUserTypeSeriesSet, fitDataToSkadiChart, odinToSkadiChart, paramSetLineStyle, updatePlotTraceName, WodinPlotData } from "./plot";
...
export const getPlotData = <T>(
  ...
  } else if (type === DataType.Fit) {
    return getFitTraceData(ctx as ActionContext<T, FitState>, cfg);
  } else if (type === DataType.Sensitivity) {
...
const getSensitivitySummaryData = <T>(
  ...
  if (type === DataType.SensitivityValueAtTime) {
    getSeriesFromBatch = batch => batch.valueAtTime(plotSettings.time!);
  } else {
    const paramPrefix = type === DataType.SensitivityTimeAtExtreme
