...
export default defineComponent({
    ...
    setup(props) {
        ...
        const parametersToShow = computed(() =>
            store.state.run.showUnchangedParameters
                ...
                : Object.fromEntries(
                      Object.entries(props.parameterSet.parameterValues).filter(
                          ([name, value]) =>
                              currentParams.value[name] !== undefined && value !== currentParams.value[name]
                      )
                  )
        );
        ...
        const lineStyleClass = computed(() => `trace-${paramSetLineStyle(props.index).split(" ").join("-")}`);
