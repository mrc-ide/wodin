<template>
    <div class="container mb-4">
        <div class="row mb-3" id="sensitivity-plot-type">
            ...
            <div class="col-11">
                <select class="form-select" v-model="plotType">
                    <option v-for="option in plotTypeOptions" :value="option.value" :key="option.value">
                        {{ option.label }}
                    </option>
                </select>
            </div>
        ...
        <div v-if="timePlotType" id="sensitivity-plot-time" class="row mb-2">
            ...
            <div class="col-xl-6 col-11">
                <numeric-input :value="time" :min-allowed="0" @update="(n) => (time = n)"></numeric-input>
            </div>
        </div>
    </div>
</template>
...
export default defineComponent({
    ...
    setup() {
        ...
        const time = computed({
            ...
            set: (newVal) => {
                // clip to 0 to endTime range
                const clampedVal = Math.max(0, Math.min(newVal, modelEndTime.value));
                store.commit(`${namespace}/${SensitivityMutation.SetPlotTime}`, clampedVal);
            }
        });
